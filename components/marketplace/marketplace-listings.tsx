...
export function MarketplaceListings({
    ...
    const fetchListings = async () => {
      try {
        ...
        const data = await readContract({
          ...
          params: [BigInt(0), BigInt(Math.min(count, 10))],
        });
        ...
        for (let i = 0; i < Math.min(count, 10); i++) {
          try {
            ...
            const isActive =
              Number(listingData.startTime) <= now &&
              now <= Number(listingData.endTime);
        ...
        console.error("error from fetching listings", err);
        setError(
  ...
  const handleBuy = async (listingId: string) => {
    ...
    try {
      ...
      const totalPrice =
        BigInt(listing.pricePerToken) * BigInt(listing.quantity);
